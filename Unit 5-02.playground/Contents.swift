...
class ViewController : UIViewController {
    ...
    let numbers = [Int((100) + 1), Int(arc4random_uniform(100) + 1), Int(arc4random_uniform(100) + 1)]
    
    @objc func showBiggest() {
        // print highest number
        answerL2.text = "The highest number is: \(findBiggest(numbers: numbers))"
    }
    
    @objc func findBiggest(numbers: [Int]) -> Int {
        //show total volume to the user
        let biggestNumber = numbers.max()!
